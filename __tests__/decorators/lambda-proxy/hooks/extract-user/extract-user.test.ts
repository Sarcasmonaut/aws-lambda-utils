...
describe("extractUser tests", () => {
  ...
  it("should set user undefined if source invalid", async (done) => {
    ...
    const event: any = {};
    const context: any = {};
    await extractUser({args: [event, context], decoratedFunction: null, userOpts: opts});
  ...
  it("should be able to extract sub for userSource = cognito ", async (done) => {
    ...
    const event: any = {requestContext: {authorizer: {claims: {sub: expectedUser}}}};
    const context: any = {};
    await extractUser({args: [event, context], decoratedFunction: null, userOpts: opts});
  ...
  it("should be able to extract principalId for userSource = principalId ", async (done) => {
    ...
    const expectedUser = 'user';
    const event: any = {requestContext: {authorizer: {principalId: expectedUser}}};
    const context: any = {};
    await extractUser({args: [event, context], decoratedFunction: null, userOpts: opts});
    expect(event).toHaveProperty('user', expectedUser);
    done();
  ...
  test.each([['NODE_ENV', 'test'], ['IS_OFFLINE', 'true']])("should set user to LOCAL_USER if process.env.%p is %p ", async (envKey, val) => {
    const opts: LambdaProxyOpts = {userSource: 'principalId'};
    const event: any = {requestContext: {}};
    const context: any = {};
