...
describe("BodyParser.parseJsonString tests", () => {
  ...
  test.each(['notajsonstring', undefined])("[%p] should raise BadRequestError if invalid body was provided ", (invalidBody: any) => {
    // @ts-ignore
    expect(() => BodyParser.parseJsonString(invalidBody)).toThrow(new BadRequestError('Malformed Body. Expected stringified json content.'));
  });
