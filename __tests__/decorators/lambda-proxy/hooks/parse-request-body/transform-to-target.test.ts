...
describe("BodyParser.transformToTarget tests", () => {
  ...
  test.each([true, false])(
    "will call plainToClass with body, opts.type and opts.strict=%p",
    (strict) => {
  ...
  test.each([false, null, undefined, 0])(
    "[ %p ] will not remove undefined values if opts.stripUndefined falsey",
    (strip: any) => {
  ...
  test.each([1, "something", true, [1]])(
    "[ %p ] will remove undefined values if opts.stripUndefined truthy",
    (strip: any) => {
      const opts: ParseBodyOpts = {
        type: TestDto,
        stripUndefined: strip,
  ...
  it("should only return exposed fields if strict is true", () => {
    ...
    const body = {
      exposed: "I shall prevail",
      notExposed: "I will perish",
      unknown: "I shouldn`t even be here",
    };
