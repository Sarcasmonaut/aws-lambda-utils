...
describe("BodyParser.validateEvent tests", () => {
  ...
  test.each([
    ...
    "should return %p if with method %p",
    (expected: boolean, httpMethod: string) => {
      const event = {
        httpMethod,
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ message: "bla" }),
      };
      // @ts-ignore
  ...
  test.each([
    ...
    "should return %p if unsupported Content-Type %p",
    (expected: boolean, type: string) => {
      const event = {
        httpMethod: "POST",
