...
describe("ResponseTransformer.setStatus tests", () => {
  ...
  test.each([
    ...
    "should set statusCode %p if provided with opts.returns: %p",
    (expectedCode: number, returns: any) => {
  ...
  test.each([
    ...
    "should set statusCode %p if params.error = %p ",
    (expectedCode: number, error: any) => {
  ...
  test.each([
    ...
    "should set statusCode %p if params = %p",
    (expectedCode: number, params: any) => {
      ResponseTransformer.setStatus(params as any);
      expect(params).toHaveProperty("result");
      expect(params.result).toHaveProperty("statusCode", expectedCode);
    }
